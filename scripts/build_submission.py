...
DATA_AVAILABILITY = {
    "paper2": "The three fetchers, the domain adapters, the validation harness "
              "and the per-domain statistics are in the repository, and every "
              "figure is generated by a script included with it. The collected "
              "source text is not redistributed: all three registers are public "
              "APIs that a reader can call with their own key, and the fetchers "
              "name the endpoints and parameters needed to do so. API keys are "
              "read from a file that is not part of the repository.",
